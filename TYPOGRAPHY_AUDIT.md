...
В проект внедрена одна финальная типографическая система на Montserrat. Старый слой текстовых токенов, временные алиасы, локальные размеры в пикселях, локальные `clamp()` для текста, старые Sass-миксины типографики и случайные `line-height` / `letter-spacing` удалены из `src`.
...
- Локальные `font-size` в пикселях в `src` не используются.
...
- Локальные `font-size` в пикселях в `src`.
...
- нет локальных `font-size` в пикселях в `src`;
